        ...
        <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 space-x-0 md:space-x-4">
          {
            sessionStatus !== "authenticated" ? (
              <button onClick={() => { signIn("google", { callbackUrl: "/dashboard" }) }} className="z-10 inline-flex gap-1 h-12 animate-shimmer items-center justify-center rounded-md border border-[#112f23] bg-[linear-gradient(110deg,#1e1e20,45%,#112f23,55%,#1e1e20)] bg-[length:200%_100%] px-6 font-medium text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
                Sign in
              </button>
            ) : (
              <button onClick={() => { redirect("/dashboard") }} className="inline-flex gap-1 h-12 animate-shimmer items-center justify-center rounded-md border border-[#112f23] bg-[linear-gradient(110deg,#1e1e20,45%,#112f23,55%,#1e1e20)] bg-[length:200%_100%] px-6 font-medium text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
                Dashboard
              </button>
            )
          }
